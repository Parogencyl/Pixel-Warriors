...
$selectSlotsBp = "SELECT backpack.slot1, backpack.slot2, backpack.slot3, backpack.slot4, backpack.slot5, backpack.slot6,
    ...
    for ($i = 1; $i <=9; $i++) {
        if ($rowBp['slot'.$i] == "../Przedmioty/Ramka_Gif.gif") {
            $dropItem = "UPDATE backpack INNER JOIN player ON player.Id=backpack.IdBackpack SET backpack.Slot$i='$slot7' WHERE player.Login='$user_check'";
    ...
    // Aktualizacja eq po zdjęciu
        if ($row['slot7'] != "../Przedmioty/Ramka_Gif.gif") {
            $updateEq = "UPDATE equipment INNER JOIN player ON player.Id=equipment.IdEquipment SET equipment.Slot7='../Przedmioty/Ramka_Gif.gif' WHERE player.Login='$user_check'";
            ...
            MagicDefence='0', Improvment='0', ItemId='1', Price='0'  WHERE player.Login='$user_check' AND Slot='7'";
