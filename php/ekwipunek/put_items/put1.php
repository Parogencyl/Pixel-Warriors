...
$selectSlotsBp = "SELECT backpack.slot1, backpack.slot2, backpack.slot3, backpack.slot4, backpack.slot5, backpack.slot6,
    ...
    // Aktualizacja eq po założeniu
    if ($row['slot1'] == "../Przedmioty/Ramka_Gif.gif") {
        $itemSlot = $rowBp['slot'.$number];
        $updateEq = "UPDATE equipment INNER JOIN player ON player.Id=equipment.IdEquipment SET equipment.Slot1='$itemSlot' 
        ...
        $getSlotsEq = mysqli_query($connection, $updateEq);
            ...
            MagicDefence='$mDefence', Improvment='$improvment', ItemId='$itemId', Price='$price' WHERE player.Login='$user_check' AND Slot='1'";
            $setStatBpQuery = mysqli_query($connection, $setStatEq);
        
    }

    // Aktualizacja plecaka po założeniu
    
    if ($rowBp['slot'.$number] != "../Przedmioty/Ramka_Gif.gif") {
        $itemSlotBp = $rowBp['slot'.$number];
        $dropItem = "UPDATE backpack INNER JOIN player ON player.Id=backpack.IdBackpack SET backpack.Slot$number='../Przedmioty/Ramka_Gif.gif' 
        WHERE player.Login='$user_check'";
