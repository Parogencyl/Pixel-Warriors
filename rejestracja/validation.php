...
<?php
    ...
    $getLogins = "SELECT player.Login FROM player";
    $getLoginsQuery = mysqli_query($connection, $getLogins);
    if(mysqli_num_rows($getLoginsQuery)>0){
        while($row = mysqli_fetch_assoc($getLoginsQuery)){
...
<script>
    ...
    // Skopiowanie tablicy loginów
    var loginsArray = <?php echo json_encode($loginPlayers); ?>;
    ...
    function loginCheck() {
        ...
        if (login == "") {
            ...
            for(let i=0; i<loginsArray.length; i++){
                if(loginsArray[i] == login){
                    document.getElementById("badLogin").style.display = "block";
                    document.getElementById("badLogin").innerHTML += "- Ten login jest zajęty <br>";
                    errorLogin++;
                }
            }
        }
